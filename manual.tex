...
\begin{landscape}
	...
	%	\resizebox{!}{0.75\textheight}{
			\begin{circuitikz}
				%\node[anchor=center,inner sep=0] at (1.25,0.6) {\includegraphics[width=1.5\textwidth]{/home/niklas/Pictures/Screenshots/sc.png}};
				
				% components
				\node[regfile, align=center, no leads] (rf) {Register\\File};
				\node[instrmem, align=center, left=2cm of rf, no leads] (im) {Instruction\\Memory};
				\node[alu, align=center, right=2.25cm of rf.rd1, anchor=a, no leads] (alu) {ALU};
				\node[datamem, align=center, right=6.25cm of rf, no leads] (dm) {Data\\Memory};
		
				\node[ctrlunitsc, align=center, blue, above=1cm of rf, xshift=-0.75cm, no leads] (cu) {Control\\Unit};
				
				\node[extend, align=center, below=1cm of rf, no leads] (ext) {Extend};
				\node[adder, align=center, anchor=b, no leads] (addpctarget) at (ext.out -| alu.a) {};
				\node[adder, align=center, anchor=a, no leads] (addpcplus4) at (ext.in -| im.a) {};
		
				% TODO: aling mux.out and alu.b
				\node[mux, align=center, right=1cm of rf.rd2, anchor=in0, no leads] (muxalusrc) {};
				\node[reg, align=center, left=0.3cm of im.a, no leads] (regpcnext) {};
				\node[mux, align=center, left=1.25cm of regpcnext, no leads] (muxpcnext) {};
				\node[mux, align=center, right=1cm of dm.rd, anchor=in1, no leads] (muxresult) {};
				
				% special nodes
				% clks
				\node[align=center, above=0.25cm of regpcnext.clk, inner sep=1pt] (clk1) {CLK};
				\draw[] (clk1) -- (regpcnext.clk);
				\node[align=center, above=0.25cm of rf.bclk, inner sep=1pt] (clkrf) {CLK};
				\draw[] (clkrf) -- (rf.bclk);
				\node[align=center, above=0.25cm of dm.bclk, inner sep=1pt] (clkdm) {CLK};
				\draw[] (clkdm) -- (dm.bclk);
				
				% constants
				\node[align=right, left=0.25cm of addpcplus4.bb,inner sep=1pt] (const4) {4};
...
%				\draw[] (dm.rd) -- (muxresult.in1) node[align=center,above, pos=0.5] {ReadData};
%				\draw[] (alu.out) -- ++(2,0) -- ++(0, 2.5) -- ++(4.5,0) |- (muxresult.in0);
...
%				\draw[] (ext.out) ++(0.25, 0)|- (muxalusrc.in1);
%				
%				% instruction memory
...
%				\draw[] (addpcplus4.out) -- ++(0, -2)  -- ++ (-7,0)  node[align=center,above, pos=0.5] {PCPlus4} |- (muxpcnext.in0);
%				\draw[] (addpctarget.out) -- ++(0, -3)  -- ++ (-18.5,0)  node[align=center,above, pos=0.5] {PCTarget} |- (muxpcnext.in1);
%				
			\end{circuitikz}
	%}
	\end{center}
