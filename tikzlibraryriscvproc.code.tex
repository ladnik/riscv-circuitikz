...
\ctikzset{processor/memory/height/.initial=3}				% height of memory components
\ctikzset{processor/memory/width/.initial=1.75}				% width of memory components extend unit
\ctikzset{processor/memory/tickwidth/.initial=2.25}			% width of register file

\ctikzset{processor/control/height/.initial=4}				% height of control unit
\ctikzset{processor/control/width/.initial=1.3}				% width of control unit
\ctikzset{processor/control/radius/.initial=10}				% border radius fo control unit

\ctikzset{processor/arith/height/.initial=1.5}				% height of arith components (e.g. alu/adder)
\ctikzset{processor/arith/width/.initial=1}					% width of arith components
\ctikzset{processor/arith/slope/.initial=15}				% slope of arith component sides

\ctikzset{processor/extend/slope/.initial=10}				% slope of extend unit side

\ctikzset{processor/mux/slope/.initial=15}					% slope of mux sides

\ctikzset{processor/misc/smallwidth/.initial=0.4}			% width for small components (e.g. small reg/mux/clk input)
\ctikzset{processor/misc/smallheight/.initial=0.8}			% height for small components (e.g. small reg/2-1-mux, right side of extend)
...
% Defines some useful saved dimensions and macros. Note: These are not global macros, i.e. they do not persist across scopes and need to be redefined.
%\def\definedimensions{
%	\savedmacro{\ctikzclass}{\edef\ctikzclass{processor}}
%
%	\pgfmathparse{\ctikzvalof{processor/scale}\pgf@circ@Rlen}
%	\pgfmathsetmacro{\scaledRlen}{\pgfmathresult pt}
%
%	\pgfmathparse{\ctikzvalof{processor/scale} * \scaledRlen}
%	\saveddimen{\leadlen}{\pgfmathsetlength{\pgf@x}{\pgfmathresult pt}}
%}
...
\pgfdeclareshape{regfile}{
	...
	\pgf@circ@draw@component{
		...
		\ifpgfcirc@draw@input@leads
			...
			% a2
			\pgfpathmoveto{\pgfpoint{\pgf@circ@res@left-\leadlen}{0.3\pgf@circ@res@up}}
		...
		\ifpgfcirc@draw@output@leads
			...
			% rd2
			\pgfpathmoveto{\pgfpoint{\pgf@circ@res@right+\leadlen}{0.3\pgf@circ@res@up}}
